...
// ▼ Functions
float3 toning(float3 col) {
	float3 i = float3(.3, .6, .1);
	float lum = col.r*i.r + col.g*i.g + col.b*i.b;
	float3 tone = float3(0.7, 0.94, 1.0);
	return saturate(float3(lum*tone.r, lum*tone.g, lum*tone.b));
}
// ▲ Functions
...
void main(in PS_Input psi, out PS_Output PSOutput)
{
	...
	color = TEXTURE_0.Sample( TextureSampler0, psi.uv );
#else
	color = texture2D_AA(TEXTURE_0, TextureSampler0, psi.uv);
#endif

#ifdef MASKED_MULTITEXTURE
	float4 tex1 = TEXTURE_1.Sample(TextureSampler1, psi.uv);
	...
	testColor.a = testColor.a * psi.alphaTestMultiplier.r;
	...
	float4 tex1 = TEXTURE_1.Sample(TextureSampler1, psi.uv);
	float4 tex2 = TEXTURE_2.Sample(TextureSampler2, psi.uv);
	...
	float4 tintTex = TEXTURE_1.Sample(TextureSampler1, psi.uv);
	...
	color.rgb = lerp( color, psi.overlayColor, psi.overlayColor.a ).rgb;
#endif

#ifdef USE_EMISSIVE
	//make glowy stuff
	color *= lerp( float( 1.0 ).xxxx, psi.light, color.a );
#else
	color *= psi.light;
#endif

/*+:｡. Tempest Shaders Entity shading .｡:+*/
color.rgb = lerp(color.rgb*1.3, toning(color.rgb), .23);
/*+:^\ ▲ Tempest Shaders Entity shading END ▲ /^:+*/

	//apply fog
	color.rgb = lerp( color.rgb, psi.fogColor.rgb, psi.fogColor.a );

#ifdef GLINT
	// Applies color mask to glint texture instead and blends with original color
	float4 layer1 = TEXTURE_1.Sample(TextureSampler1, frac(psi.layerUV.xy)).rgbr * GLINT_COLOR;
	float4 layer2 = TEXTURE_1.Sample(TextureSampler1, frac(psi.layerUV.zw)).rgbr * GLINT_COLOR;
